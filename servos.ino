...
{
  // PWM Pin 0
  // if      (config.channel[0].motorMarker == MOTOR)  output0.attach( output0_Pin, ONESHOTFREQ, ONESHOTMIN, ONESHOTMAX);
  // else if (config.channel[0].motorMarker == ASERVO) output0.attach( output0_Pin, ANALOGFREQ,  ANALOGMIN,  ANALOGMAX);
  // else                                              output0.attach( output0_Pin, DIGITALFREQ, DIGITALMIN, DIGITALMAX);
  if (config.channel[0].motorMarker == MOTOR) {
    output0.attach( output0_Pin, ONESHOTFREQ, ONESHOTMIN, ONESHOTMAX);
  } else if (config.channel[0].motorMarker == ASERVO) {
    output0.attach( output0_Pin, ANALOGFREQ,  ANALOGMIN,  ANALOGMAX);
  } else {
    output0.attach( output0_Pin, DIGITALFREQ, DIGITALMIN, DIGITALMAX);
  }

  // PWM Pin 1
  if (config.channel[1].motorMarker == MOTOR) {
    output1.attach( output1_Pin, ONESHOTFREQ, ONESHOTMIN, ONESHOTMAX);
  } else if (config.channel[1].motorMarker == ASERVO) {
    output1.attach( output1_Pin, ANALOGFREQ,  ANALOGMIN,  ANALOGMAX);
  } else {
    output1.attach( output1_Pin, DIGITALFREQ, DIGITALMIN, DIGITALMAX);
  }

  // PWM Pin 2
  if (config.channel[2].motorMarker == MOTOR) {
    output2.attach( output2_Pin, ONESHOTFREQ, ONESHOTMIN, ONESHOTMAX);
  } else if (config.channel[2].motorMarker == ASERVO) {
    output2.attach( output2_Pin, ANALOGFREQ,  ANALOGMIN,  ANALOGMAX);
  } else {
    output2.attach( output2_Pin, DIGITALFREQ, DIGITALMIN, DIGITALMAX);
  }

  // PWM Pin 3
  if (config.channel[3].motorMarker == MOTOR) {
    output3.attach( output3_Pin, ONESHOTFREQ, ONESHOTMIN, ONESHOTMAX);
  } else if (config.channel[3].motorMarker == ASERVO) {
    output3.attach( output3_Pin, ANALOGFREQ,  ANALOGMIN,  ANALOGMAX);
  } else {
    output3.attach( output3_Pin, DIGITALFREQ, DIGITALMIN, DIGITALMAX);
  }

// PWM Pin 4
  if (config.channel[4].motorMarker == MOTOR) {
    output4.attach( output4_Pin, ONESHOTFREQ, ONESHOTMIN, ONESHOTMAX);
  } else if (config.channel[4].motorMarker == ASERVO) {
    output4.attach( output4_Pin, ANALOGFREQ,  ANALOGMIN,  ANALOGMAX);
  } else {
    output4.attach( output4_Pin, DIGITALFREQ, DIGITALMIN, DIGITALMAX);
  }

// // PWM Pin 5
  if (config.channel[5].motorMarker == MOTOR) {
    output5.attach( output5_Pin, ONESHOTFREQ, ONESHOTMIN, ONESHOTMAX);
  } else if (config.channel[5].motorMarker == ASERVO) {
    output5.attach( output5_Pin, ANALOGFREQ,  ANALOGMIN,  ANALOGMAX);
  } else {
    output5.attach( output5_Pin, DIGITALFREQ, DIGITALMIN, DIGITALMAX);
  }

#if defined(ARDUINO_GENERIC_F405RGTX)

  // PWM Pin 6
  if (config.channel[6].motorMarker == MOTOR) {
    output6.attach( 7, ONESHOTFREQ, ONESHOTMIN, ONESHOTMAX);
  } else if (config.channel[6].motorMarker == ASERVO) {
    output6.attach( 7, ANALOGFREQ,  ANALOGMIN,  ANALOGMAX);
  } else {
    output6.attach( 7, DIGITALFREQ, DIGITALMIN, DIGITALMAX);
  }

  // PWM Pin 7
  if (config.channel[7].motorMarker == MOTOR) {
    output7.attach(14, ONESHOTFREQ, ONESHOTMIN, ONESHOTMAX);
  } else if (config.channel[7].motorMarker == ASERVO) {
    output7.attach(14, ANALOGFREQ,  ANALOGMIN,  ANALOGMAX);
  } else {
    output7.attach(14, DIGITALFREQ, DIGITALMIN, DIGITALMAX);
  }

  // PWM Pin 8
  if (config.channel[8].motorMarker == MOTOR) {
    output8.attach(24, ONESHOTFREQ, ONESHOTMIN, ONESHOTMAX);
  } else if (config.channel[8].motorMarker == ASERVO) {
    output8.attach(24, ANALOGFREQ,  ANALOGMIN,  ANALOGMAX);
  } else {
    output8.attach(24, DIGITALFREQ, DIGITALMIN, DIGITALMAX);
  }

// PWM Pin 9
  if (config.channel[9].motorMarker == MOTOR) {
    output9.attach(28, ONESHOTFREQ, ONESHOTMIN, ONESHOTMAX);
  } else if (config.channel[9].motorMarker == ASERVO) {
    output9.attach(28, ANALOGFREQ,  ANALOGMIN,  ANALOGMAX);
  } else {
    output9.attach(28, DIGITALFREQ, DIGITALMIN, DIGITALMAX);
  }
